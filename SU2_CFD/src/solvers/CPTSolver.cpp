    ...
    if(foundEulerWall){
      ...
      for (iVertex = 0; iVertex < geometry_fine->nVertex[val_marker]; iVertex++) {
        ...
        if(nDim==2){
          ...
          u_mag +=V_domain[1]*V_domain[1] + V_domain[2]*V_domain[2] + V_domain[3]*V_domain[3];
        }
        ...
        if(LWC>LWC_threshold && projvelocity<0 && u_mag>u_threshold){
          if(nDim==2){
